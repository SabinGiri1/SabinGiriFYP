...
const RelatedVets = ({ speciality, vetId }) => {
  const { vets } = useContext(AppContext);
  const navigate = useNavigate();

  const [relVet, setRelVets] = useState([]);

  useEffect(() => {
    if (vets.length > 0 && speciality) {
      const vetsData = vets.filter((vet) => vet.speciality === speciality && vet._id !== vetId);
      setRelVets(vetsData);
    }
  }, [vets, speciality, vetId]);
  ...
  return (
    <div className='flex flex-col items-center gap-6 my-16 text-gray-900 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 pt-5 px-3 sm:px-0'>
        {relVet.slice(0, 5).map((item, index) => (
          <div
